...
export default function RubAzn() {
  const { t } = useTranslation();
  const lp = useLocalePath();
  return (
    <>
      <Helmet>
        <title>{`${t("converter.pageTitleRub")} | Pultap.az`}</title>
        <meta name="description" content={t("converter.metaRub")} />
      </Helmet>
      <section className="bg-gradient-hero text-primary-foreground">
        <div className="container py-10 md:py-14">
          <nav className="flex items-center gap-1.5 text-xs text-primary-foreground/70 mb-3">
            ...
            <ChevronRight className="h-3 w-3" />
            <span>{t("converter.pairs.rub")}</span>
          </nav>
          <h1 className="text-3xl md:text-4xl font-display font-bold tracking-tight">{t("converter.pageTitleRub")}</h1>
        </div>
      </section>
      <section className="container py-10 md:py-14">
        <Converter from="RUB" to="AZN" title="RUB → AZN" />
      </section>
    </>
  );
}
