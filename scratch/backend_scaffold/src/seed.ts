...
const SUPABASE_URL = 'https://skhzckfbqecahidcogyk.supabase.co';
const SUPABASE_SERVICE_ROLE_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InNraHpja2ZicWVjYWhpZGNvZ3lrIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc3NzE0OTQxOCwiZXhwIjoyMDkyNzI1NDE4fQ.Rkw7Dhsz9IgMhq0rbQgVsmWBQGD7vOCYvW3dXmbSU7U';
...
async function seed() {
  ...
  const { error: err1 } = await supabase.from('banks').upsert(banks.map(b => ({
    id: b.id, name: b.name, logoColor: b.logoColor, rating: b.rating, reviews: b.reviews
  })));
  if (err1) console.error("Error inserting banks", err1);
  
  const { error: err2 } = await supabase.from('credits').upsert(credits.map(c => ({
    id: c.id, name: c.name, bankId: c.bankId, type: c.type, rate: c.rate, amountMin: c.amountMin, amountMax: c.amountMax, termMin: c.termMin, termMax: c.termMax, collateral: c.collateral, insurance: c.insurance, highlight: c.highlight
  })));
  if (err2) console.error("Error inserting credits", err2);

  const { error: err3 } = await supabase.from('deposits').upsert(deposits.map(d => ({
    id: d.id, name: d.name, bankId: d.bankId, rate: d.rate, currency: d.currency, termMonths: d.termMonths, minAmount: d.minAmount
  })));
  if (err3) console.error("Error inserting deposits", err3);

  const { error: err4 } = await supabase.from('cards').upsert(cards.map(c => ({
    id: c.id, name: c.name, bankId: c.bankId, kind: c.kind, cashback: c.cashback, annualFee: c.annualFee, limit: c.limit, perks: c.perks
  })));
  if (err4) console.error("Error inserting cards", err4);

  const { error: err5 } = await supabase.from('currencies').upsert(currencies);
  if (err5) console.error("Error inserting currencies", err5);

  const { error: err6 } = await supabase.from('bokts').upsert(bokts.map(b => ({
    id: b.id, name: b.name, logoColor: b.logoColor, rating: b.rating, reviews: b.reviews
  })));
  if (err6) console.error("Error inserting bokts", err6);

  const { error: err7 } = await supabase.from('bokt_products').upsert(boktProducts.map(b => ({
    id: b.id, name: b.name, boktId: b.boktId, type: b.type, rate: b.rate, amountMin: b.amountMin, amountMax: b.amountMax, termMin: b.termMin, termMax: b.termMax, collateral: b.collateral, highlight: b.highlight
  })));
  if (err7) console.error("Error inserting bokt products", err7);

  const { error: err8 } = await supabase.from('institutions').upsert(institutions.map(i => ({
    id: i.id, name: i.name, officialName: i.officialName, logoColor: i.logoColor, type: i.type, established: i.established, branches: i.branches, atms: i.atms, website: i.website, phone: i.phone, rating: i.rating, reviews: i.reviews, description: i.description, swift: i.swift, voen: i.voen, correspondentAccount: i.correspondentAccount, address: i.address, logoUrl: i.logoUrl
  })));
  if (err8) console.error("Error inserting institutions", err8);

  const { error: err9 } = await supabase.from('blog_posts').upsert(blogPosts);
  if (err9) console.error("Error inserting blog posts", err9);
